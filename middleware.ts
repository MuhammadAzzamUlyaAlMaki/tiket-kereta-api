...
export const middleware =
    async (request: NextRequest) =>{
        ...
        const isVerifyToken = await verifyKaryawan (token?? "")
        if (!isVerifyToken) return NextResponse.redirect(redirectLogin)
    ...
    if (request.nextUrl.pathname.startsWith(`/pelanggan`)) {
        ...
        const isVerifyToken = await verifyPelanggan (token?? "")
        if (!isVerifyToken) return NextResponse.redirect(redirectLogin)
            return NextResponse.next()
    }
    return NextResponse.next()
